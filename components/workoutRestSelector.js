...
const WorkoutRestSelector = ({ addRestBetweenExercisesLength }) => {
  const [setRestLength, setSetRestLength] = useState(40);
  const [minusButtonDisabled, setMinusButtonDisabled] = useState(true);

  useEffect(() => {
    setMinusButtonDisabled(setRestLength <= 5);
    addRestBetweenExercisesLength(setRestLength);
  }, [setRestLength]);
...
export default WorkoutRestSelector;
